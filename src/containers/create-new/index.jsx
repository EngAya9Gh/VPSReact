...
import { useRouter } from 'next/router'; // استيراد useRouter
  ...
  useEffect(() => {
    // كلما تغيرت العملة أو البيانات، نعيد تعيين الحقول
    setTransferOrderField((prev) => ({
      ...prev,
      transfer_money_firm_id: data ? data.id : '',
      currency, // الآن تُعين القيمة الصحيحة هنا بعد تحديث useEffect الأول
    }));
  }, [currency, data]);
